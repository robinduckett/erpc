  ...
  - id: main
    ...
    upstreams:
      ...
      - endpoint: https://arbitrum-one.blastapi.io/xxxxxxx-xxxxxx-xxxxxxx
        ...
        # (OPTIONAL) Define error patterns to ignore when recording health metrics.
        # Ignored errors won't affect error rates, circuit breakers, or Prometheus metrics.
        # This is useful for expected errors like "transaction underpriced" that don't indicate upstream problems.
        ignoreErrors:
          - message: "transaction underpriced"
            matchType: substring  # Options: exact, substring, wildcard, regex
          - message: "nonce too low"
            matchType: substring
          - code: "ErrEndpointCapacityExceeded"
            matchType: exact
          - message: "transaction*underpriced"  # Wildcard pattern
            matchType: wildcard
          - message: "nonce (is )?too low"  # Regex pattern
            matchType: regex
...
projects:
  - id: main
    ...
    upstreamDefaults:
      ...
      # Default error patterns to ignore (see ignoreErrors section above for details)
      ignoreErrors:
        - message: "transaction underpriced"
          matchType: substring
        - message: "nonce too low"
...
The `ignoreErrors` configuration allows you to prevent specific error patterns from affecting upstream health metrics. This is useful for handling expected errors that don't indicate upstream problems, such as mempool transaction errors or rate limit errors.

### How it works

When an error matches an ignore pattern:
- ❌ **NOT** recorded in `ErrorsTotal` metrics
- ❌ **NOT** included in error rate calculations
- ❌ **NOT** counted toward circuit breaker thresholds
- ❌ **NOT** emitted in Prometheus error metrics
- ✅ Debug log message emitted indicating the error was ignored
- ✅ Error is still returned to the caller (not silenced at API level)

### Match types

You can use four different match types to identify errors:

- **`exact`**: Exact string match (case-sensitive)
- **`substring`**: Contains match (case-sensitive, **default**)
- **`wildcard`**: Shell-style wildcard patterns using `*` (e.g., `transaction*underpriced`)
- **`regex`**: Full regular expression support (e.g., `nonce (is )?too low`)

### Match targets

Errors can be matched against two fields:

- **`message`**: Matches the error message text (`error.Error()`)
- **`code`**: Matches the error code (for internal `StandardError` types)
- Both fields can be used together (AND logic - both must match)

### Configuration levels

Error patterns can be configured at three levels (in order of precedence):
1. **Upstream-specific** (`upstreams[].ignoreErrors`) - highest priority
2. **Network-wide** (`networkDefaults.ignoreErrors`) - medium priority
3. **Project-wide** (`upstreamDefaults.ignoreErrors`) - lowest priority

<Callout type='info'>
  Patterns from all three levels are combined, with upstream-specific patterns taking the highest priority.
</Callout>

### Common use cases

**Ignore mempool transaction errors:**
<Tabs items={["yaml", "typescript"]} defaultIndex={0} storageKey="GlobalConfigTypeTabIndex">
  <Tabs.Tab>
```yaml
upstreams:
...
upstreams: [
  {
    id: "my-rpc",
    ignoreErrors: [
      { message: "transaction underpriced", matchType: "substring" },
      { message: "nonce too low", matchType: "substring" },
      { message: "replacement transaction underpriced", matchType: "substring" },
    ],
  },
]
```
...
upstreams: [
  {
    id: "my-rpc",
    ignoreErrors: [
      { code: "ErrEndpointCapacityExceeded", matchType: "exact" },
      { message: "rate limit", matchType: "substring" },
    ],
  },
]
```
...
upstreams: [
  {
    id: "my-rpc",
    ignoreErrors: [
      { code: "ErrEndpoint*", matchType: "wildcard" },
      { message: "transaction*underpriced", matchType: "wildcard" },
    ],
  },
]
```
